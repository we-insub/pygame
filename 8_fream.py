...
# 1. 사용자 게임 초기화 ( 배경, 게임이미지 , 좌표 , 이동속도 ,폰트등 )
...
enemy_x_pos = (screen_width / 2) -(enemy_width /2)# 화면 가로의 절반크기에 해당위치하도록
enemy_y_pos = (screen_height / 2) -(enemy_height/2) # 화면 세로의 가장아래에 해당위치
    ...
    for event in pygame.event.get():  # 이벤트 루프라는것은 프로그램이 종료되지 않게 대기 함으로 중간에 마우스 키보드 어떤 동작이 들어오는지 확인
        ...
        if event.type == pygame.KEYDOWN: # 키가 눌렸는지 확인
            if event.key == pygame.K_LEFT:
                ...
                to_y -= character_speed  # to_x = to_x - 3
